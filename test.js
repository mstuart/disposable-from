...
const noop = () => {
  // Intentionally empty callback used by disposal lifecycle tests.
};

// DisposableTimer tests

test("disposableTimer creates a disposable", (t) => {
  const timer = disposableTimer(noop, 100_000);
...
test("disposableInterval creates a disposable", (t) => {
  const interval = disposableInterval(noop, 100_000);
...
test("disposableListener is safe to dispose multiple times", (t) => {
  const target = new EventTarget();
  const listener = disposableListener(target, "test", noop);
  listener[Symbol.dispose]();
...
test("disposableListener passes options", (t) => {
  const target = new EventTarget();
  let count = 0;
  const listener = disposableListener(
    target,
    "test",
    () => {
      count += 1;
...
test("all functions return objects with Symbol.dispose", (t) => {
  ...
  const interval = disposableInterval(noop, 100_000);
  const listener = disposableListener(target, "test", noop);
  const callback = disposableCallback(() => noop);
